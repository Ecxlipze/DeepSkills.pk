...
export default async function handler(req, res) {
  ...
  const auth = await authorizeAdminOperation(req, 'users');
  if (!auth.ok) {
    return res.status(auth.status).json({ status: 'error', message: auth.message });
  }

  const supabase = getSupabaseServerClient();
  const { action = 'sync', cnic, name, role = 'custom', assignedCourse, course, batch } = req.body || {};
  ...
  try {
    if (action === 'revoke') {
      ...
      return res.status(200).json({ status: 'success', message: 'Staff login access revoked.' });
    }

    if (action === 'sync') {
      const { data: existingAllowed } = await supabase.from('allowed_cnics').select('role, name').eq('cnic', formattedCnic).maybeSingle();
      if (existingAllowed && ['student', 'teacher'].includes(existingAllowed.role)) {
      ...
      if (error) throw error;
      return res.status(200).json({ status: 'success', message: 'Staff login access synchronized.' });
    }
