...
export function hrHandler(action) {
  return async function handler(req, res) {
    ...
    try {
      ...
      const teacher = profile.teacher_id ? await first(db, 'teachers', 'id', profile.teacher_id) : null;
      if (profile.teacher_id && !teacher) throw failure(404, 'Teacher record not found.');
      const user = (!profile.teacher_id && profile.user_id) ? await first(db, 'users', 'id', profile.user_id) : null;
      if (!profile.teacher_id && profile.user_id && !user) throw failure(404, 'Staff record not found.');
      if (!teacher && !user) throw failure(404, 'Candidate record not found.');

      const candidateCnic = teacher ? teacher.cnic : user.cnic;
      const candidateStatus = teacher ? teacher.status : user.status;
      const isStaffCandidate = Boolean(user && !teacher);

      if (teacherSession && (normalizeCnic(candidateCnic) !== teacherSession.cnic || !['Active','Pending','Onboarding','active','pending','onboarding'].includes(candidateStatus))) throw failure(403, 'You can only access your own active HR application.');
      const now = new Date().toISOString();
      const name = (teacher ? teacher.name : user.full_name) || profile.full_name || (isStaffCandidate ? 'Staff Member' : 'Teacher');
      const recipient = (teacher ? teacher.email : user.email) || profile.personal_email;
      const adminRecipient = process.env.HR_EMAIL_TO || process.env.CONTACT_EMAIL_TO || 'info@deepskills.pk';
      const portal = new URL(isStaffCandidate ? '/staff/onboarding/' : '/teacher/hr/', process.env.NEXT_PUBLIC_SITE_URL || 'https://deepskills.pk').href;
      ...
      if (action === 'notify-admin') {
        ...
        for (const file of files) {
          const prefix = isStaffCandidate ? `staff-${user.id}/profile-${profile.id}/` : `teacher-${teacher.id}/profile-${profile.id}/`;
        ...
        content = `Your job description for ${escapeHtml(jd.position_title || (teacher ? teacher.specialization : profile.specialization) || (isStaffCandidate ? 'your staff role' : 'your teaching role'))} is ready to review and approve in your portal.`;
        ...
        const candidateId = teacher ? teacher.id : user.id;
        if (profile.hr_status === 'hired') return res.status(200).json({ status: 'success', teacherId: candidateId, already_finalized: true, email_sent: null, message: 'Hiring was already finalized; no duplicate email was sent.' });
        const cnic = normalizeCnic(candidateCnic);
        if (!cnic) throw failure(409, `${isStaffCandidate ? 'Staff' : 'Teacher'} CNIC is invalid. Correct it before finalizing.`);
        const jd = await first(db, 'hr_jds', 'hr_profile_id', profileId);
        ...
        if (teacher) {
          ...
          await result(db.from('users').update({ status: 'active', updated_at: now }).eq('id', user.id));
          await result(db.from('allowed_cnics').upsert({ cnic, name, role: user.role || 'custom', assigned_course: jd?.position_title || 'Staff', batch: '' }, { onConflict: 'cnic' }));
        }
