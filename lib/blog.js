...
export function mergeBlogCategories(...groups) {
  return [...new Set([...BLOG_CATEGORIES, ...groups.flat()].filter((name) => typeof name === 'string' && name.trim() && name !== 'All'))];
}

export async function fetchBlogCategories() {
  const supabase = getSupabaseServerClient();
  if (!supabase) return BLOG_CATEGORIES;
  const { data, error } = await supabase.from('blog_categories').select('name').order('name');
  if (error) {
    console.error('Could not load blog categories:', error.message);
    return BLOG_CATEGORIES;
  }
  return mergeBlogCategories((data || []).map((row) => row.name));
}
...
export const CATEGORY_FALLBACK_IMAGES = {
  ...
  Education: 'https://images.unsplash.com/photo-1531482615713-2afd69097998?auto=format&fit=crop&w=1200&q=80',
  General: 'https://images.unsplash.com/photo-1517245386807-bb43f82c33c4?auto=format&fit=crop&w=1200&q=80'
};

export function getCategoryFallbackImage(category = 'General') {
...
export function normalizePost(post = {}) {
  ...
  return {
    ...
    coverImage: post.cover_image || getCategoryFallbackImage(category),
    category,
    tags: post.tags || [],
