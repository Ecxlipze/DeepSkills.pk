...
export const MODULE_KEYS = [
  ...
  'referral', 'reports', 'hr', 'users', 'settings', 'time_tracking'
];
...
export async function findAccountByCnic(supabase, cnic) {
  ...
  if (role === 'student') {
    ...
    if (profile && !['active', 'onboarding', 'pending'].includes(String(profile.status || '').toLowerCase())) {
      return {
        ok: false,
        status: 403,
...
export async function buildUserPayload(supabase, cnic) {
  ...
  if (!['active', 'onboarding', 'pending'].includes(String(directoryUser.status || '').toLowerCase())) {
    throw new Error(`Your account is ${directoryUser.status}. Please contact the administrator.`);
  }

  let customRole = Array.isArray(directoryUser.custom_roles)
    ? directoryUser.custom_roles[0]
    : directoryUser.custom_roles;

  if (!customRole && directoryUser.custom_role_id) {
    try {
      const { data: roleRow } = await supabase
        .from('custom_roles')
        .select('id, name, color, icon, permissions')
        .eq('id', directoryUser.custom_role_id)
        .maybeSingle();
      if (roleRow) {
        customRole = roleRow;
      }
  ...
  let hrSpecialization = null;
  try {
    const { data: hrRow } = await supabase
      .from('hr_profiles')
      .select('specialization, employee_type')
      .eq('user_id', directoryUser.id)
      .maybeSingle();
    if (hrRow?.specialization) {
      hrSpecialization = hrRow.specialization;
    }
  } catch (_) {}

  const customRoleName = customRole?.name || hrSpecialization || directoryUser.designation || null;
  const roleName = customRoleName || (directoryUser.role === 'admin' ? 'Administrator' : 'Staff Member');
  ...
  if (role === 'admin') {
    ...
    const customPerms = customRole?.permissions || directoryUser.permissions || {};
  ...
  return {
    ...
    roleName,
    customRoleName,
    customRoleId: directoryUser.custom_role_id || customRole?.id || null,
    ...
    designation: directoryUser.designation || customRoleName || null,
    department: directoryUser.department || null,
...
export async function authorizeAdminOperation(req, requiredPermissionKey = null, requiredLevel = null) {
  ...
  const minLevel = requiredLevel || (['GET', 'HEAD', 'OPTIONS'].includes(req?.method) ? 'view' : 'full');
      ...
      if (userRecord) {
        ...
        if (['admin', 'super_admin', 'superadmin', 'owner'].includes(userRole)) {
          ...
          if (requiredPermissionKey) {
            ...
            const hasPerm = permissions['all'] === 'full' || keys.some(k => {
              const perm = permissions[k];
              return minLevel === 'view' ? (perm === 'view' || perm === 'full') : (perm === 'full');
            });
            if (hasPerm) {
              return { ok: true, user, role: 'custom', permissions };
            }
            return { ok: false, status: 403, message: `Insufficient permissions (requires ${minLevel} on ${keys.join(' or ')}).` };
          }
          return { ok: true, user, role: 'custom', permissions };
        }
      } else {
