-- Migration: 20260925020000_enforce_cnic_uniqueness_across_roles.sql
-- Description: Enforce strict CNIC uniqueness across Faculty (teachers), Administrative Staff (users), and Students (admissions).
-- Prevents cross-role collision, identity theft, and login whitelist hijacking in allowed_cnics.

-- 1. Upgrade enroll_counsellor_student RPC with cross-role and cross-identity CNIC guards
...
DECLARE
  ...
  v_note JSONB;
BEGIN
  IF v_cnic IS NULL OR v_name IS NULL OR v_course IS NULL THEN
    RETURN jsonb_build_object('ok', false, 'code', 'missing_required', 'message', 'Required enrollment data is missing.');
  END IF;

  -- CROSS-ROLE CNIC VALIDATION:
  -- A CNIC represents a single individual citizen and must be strictly unique across staff, students, and teachers.
  IF EXISTS (SELECT 1 FROM teachers WHERE cnic = v_cnic) THEN
    RETURN jsonb_build_object('ok', false, 'code', 'cnic_teacher_conflict', 'message', 'This CNIC is already registered to a faculty instructor. A CNIC cannot be shared across roles.');
  END IF;

  IF EXISTS (SELECT 1 FROM users WHERE cnic = v_cnic AND role NOT IN ('student')) THEN
    RETURN jsonb_build_object('ok', false, 'code', 'cnic_staff_conflict', 'message', 'This CNIC is already registered to an administrative staff member. A CNIC cannot be shared across roles.');
  END IF;

  IF EXISTS (SELECT 1 FROM allowed_cnics WHERE cnic = v_cnic AND role NOT IN ('student')) THEN
    RETURN jsonb_build_object('ok', false, 'code', 'cnic_whitelist_conflict', 'message', 'This CNIC is currently active in the portal under a non-student role.');
  END IF;

  -- Ensure student identity consistency: if student with same CNIC exists, verify name to prevent identity confusion
  IF EXISTS (
    SELECT 1 FROM admissions 
    WHERE cnic = v_cnic 
      AND (v_admission_id IS NULL OR id <> v_admission_id)
      AND LOWER(TRIM(name)) <> LOWER(TRIM(v_name))
  ) THEN
    RETURN jsonb_build_object('ok', false, 'code', 'cnic_identity_mismatch', 'message', 'This CNIC is already registered to a student with a different name. Multiple individuals cannot share the same CNIC.');
  END IF;
