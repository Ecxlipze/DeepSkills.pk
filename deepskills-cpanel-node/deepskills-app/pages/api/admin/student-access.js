...
export default async function handler(req, res) {
  ...
  const auth = await authorizeAdminOperation(req, 'students');
  if (!auth.ok) {
    return res.status(auth.status).json({ status: 'error', message: auth.message });
  }

  const supabase = getSupabaseServerClient();
  const { action = 'sync', cnic, name, course, batch } = req.body || {};
  ...
  try {
    if (action === 'revoke') {
      ...
      return res.status(200).json({ status: 'success', message: 'Student login access revoked.' });
    }
    ...
    if (action === 'sync') {
      ...
      if (error) throw error;
      return res.status(200).json({ status: 'success', message: 'Student login access synchronized.' });
    }
