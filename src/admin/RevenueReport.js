...
const RevenueReport = () => {
  ...
  const fetchRevenueReport = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/finance/revenue-report?${params.toString()}`, { headers });
      const json = await res.json();

      if (!res.ok || json.status === 'error') {
        throw new Error(json.message || 'Failed to load revenue report.');
      }

      setReportData(json.data || {});
    } catch (err) {
      console.error('Error loading revenue report:', err);
      toast.error(err.message || 'Could not fetch revenue analytics.');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  }, [preset, startDate, endDate, selectedCourse, selectedBatch, user]);
